...
\begin{tabular}{ccc}
\hline
\multicolumn{3}{c}{\textbf{L1: Ethereum}} \\
\multicolumn{3}{c}{\scriptsize базовый слой расчётов} \\
\\[-0.5em]
\multicolumn{3}{c}{$\downarrow$} \\
\\[-0.5em]
\hline
\textbf{L2: Arbitrum (USDC)} & \quad$\mid$\quad & \textbf{L2: TON (USDT)} \\
\scriptsize депозиты/выводы, rollup batches & & \scriptsize Telegram Mini App, instant \\
\\[-0.5em]
\multicolumn{3}{c}{$\downarrow$} \\
\\[-0.5em]
\hline
\multicolumn{3}{c}{\textbf{L3: Наш SMR}} \\
\multicolumn{3}{c}{\scriptsize PBFT (3 валидатора, 2/3) $\to$ Disruptor $\to$ StateExecutionHandler} \\
\multicolumn{3}{c}{\scriptsize OrderBook $\cdot$ MarginManager $\cdot$ Liquidation $\cdot$ FlatFileStore} \\
\hline
\end{tabular}
...
\begin{column}{0.48\textwidth}
\textbf{Deposit flow}\\
\small User $\to$ L2 bridge $\to$ PBFT $\to$ L3 credit
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Withdrawal flow}\\
\small User $\to$ PBFT $\to$ L2 send $\to$ L1 finality
\end{column}
\end{columns}
