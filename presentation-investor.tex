...
\begin{itemize}
    ...
    \item Один вход (API Gateway) --- единая точка отказа
...
\begin{itemize}
    ...
    \item Никакого API Gateway --- никакой единой точки отказа
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{itemize}
    ...
    \item Disruptor + OrderBook + Margin
...
\begin{enumerate}
    \item Currency model (oracle-weighted equity)
    \item Real Arbitrum Nitro + TON RPC
    \item TON Vault (multisig / contract)
    \item Validator economics (stake, slashing)
    \item Audit + Network benchmark
\end{enumerate}
\end{column}
\end{columns}

\vspace{0.3em}
\centering\footnotesize\textcolor{gray}{Детали: github.com/rgobov/dex-on-java/UNRESOLVED.md}
\end{frame}
